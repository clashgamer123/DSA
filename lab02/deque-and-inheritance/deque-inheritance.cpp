...
// TODO: Suitably initialize the queue parameters (Feel free to add any other variables as needed) 
    ...
    int resize_factor = 2;
    int back = -1; // head is at the start of the array.
    int front = 0; // tail is at the end of the array.
...
public:
    ...
    int get_capacity()
    {
        // front and back are interpreted anti clock wise.
        return this->capacity ;
    }

    int get_size()
    {
        return (front-back-1+capacity)%capacity ;
    }
...
protected:
    ...
    {
        if (isFull() and this->auto_resize = true)
        {
